...
""")
    ...
    // creating sections - what are we sorting the table view by?
    static func classificationSections() -> [[ZooAnimal]] {
        // 1. sort the class/struct ... ascending
        let sortedZooAnimals = zooAnimals.sorted { $0.classification < $1.classification }
        
        // 2. use a set to idrrntigy uniquee classifications (mammal, amphibian, etc.
        //---------each element in the Set classificationType is a section title
        let uniqueClassifications = Set(sortedZooAnimals.map {$0.classification})
        
        // 3. create an empty array of [ZooAnimals] 's array
        var sections = Array(repeating: [ZooAnimal]() , count: uniqueClassifications.count)
        //[[ZooANimal],[ZooANimal],[ZooANimal],[ZooANimal],[ZooANimal]]
        
        // 4. Create an initial starting index for the sections
        var currentIndex = 0
        
        // 5. Get current classification name
        var currentClassification = sortedZooAnimals.first?.classification
        
        // 6. loop ovwer sortedZooAnimals and place in apropriate section
